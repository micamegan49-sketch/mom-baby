...
(function () {
  ...
  MB.views.preg = function (root, params) {
    ...
    if (!preg.active) {
      root.innerHTML = `
        ...
        <div class="card">
          ...
          <p class="muted center" style="font-size:12px;margin:-6px 0 10px">— หรือ —</p>
          <div class="field"><label>วันกำหนดคลอด (ถ้าหมอแจ้งไว้)</label><input id="p-edd" type="date" /></div>
    ...
    root.innerHTML = `
      <div class="hero">
        <div class="emoji" style="padding:0;overflow:hidden">${MB.fruitImg(fruitKey, 64, { radius: 22 })}</div>
        <div style="flex:1">
      ...
      <div class="card">
        ...
        <div class="center" style="margin-bottom:8px">ขนาดเทียบ ${MB.fruitImg(fruitKey, 34, { radius: 9 })} <b>≈ ${wk.fruitName}</b> · ยาว ~${wk.len} ซม.${wk.wt ? ' · หนัก ~' + (wk.wt >= 1000 ? (wk.wt / 1000).toFixed(1) + ' กก.' : wk.wt + ' ก.') : ''}</div>
        <div class="divider"></div>
